...
local function drawBorder(x1, y1, x2, y2) -- 1 far, 2 close
  ...
  lcd.drawLine(x1, y1, x2, y2, SOLID, BLACK)
...
local function drawLandscape()
  ...
  xDispClose = ((- w - drone.x) * zScale) / z + LCD_W / 2
  drawBorder(xDispFar, yDispFar, xDispClose, yDispClose)
  lcd.drawLine(0, LCD_H/2 + 1, LCD_W - 1, LCD_H/2 + 1, SOLID, BLUE) -- horizon
end
...
local function drawLine(x1, y1, x2, y2, flag)
  if flag == '1' then
    ...
    if x2 >= LCD_W then x2 = LCD_W - 1 end
    lcd.drawLine(x1, y1, x2, y2, SOLID, ORANGE)
    return 0
  end
  ...
  if flag == '2' then
    ...
    if y2 >= LCD_H then y2 = LCD_H - 1 end
    lcd.drawLine(x1, y1, x2, y2, SOLID, DARKBROWN)
    return 0
  end
  ...
  if flag == '4' then
    ...
    if y1 >= LCD_H then y1 = LCD_H - 1 end
    if y2 >= LCD_H then y2 = LCD_H - 1 end
    lcd.drawLine(x1, y1, x2, y2, SOLID, DARKBLUE)
    return 0
  end
...
local function drawMarker(x, y)
  ...
  lcd.drawLine(x - 1, y - 1, x - 1, y + 1, SOLID, RED)
  lcd.drawLine(x    , y - 1, x    , y + 1, SOLID, RED)
  lcd.drawLine(x + 1, y - 1, x + 1, y + 1, SOLID, RED)
end
...
local function drawObject(object, markerFlag)
  ...
  if object.t == "gateGround" then
    ...
    drawLine(xDispLeft, yDispBottom, xDispLeft, yDispTop, '1')
	drawLine(xDispLeft-1, yDispBottom, xDispLeft-1, yDispTop, '1')
	
    drawLine(xDispRight, yDispBottom, xDispRight, yDispTop, '1')
	drawLine(xDispRight+1, yDispBottom, xDispRight+1, yDispTop, '1')
	
    drawLine(xDispLeft, yDispTop-1, xDispRight, yDispTop-1, '1')
	drawLine(xDispLeft, yDispTop-1, xDispRight, yDispTop-1, '1')
  ...
  elseif object.t == "gateAir" then
    ...
    drawLine(xDispLeft, yDispBottom, xDispLeft, yDispTop, '2')
	drawLine(xDispLeft-1, yDispBottom, xDispLeft-1, yDispTop, '2')
    drawLine(xDispRight, yDispBottom, xDispRight, yDispTop, '2')
	drawLine(xDispRight+1, yDispBottom, xDispRight+1, yDispTop, '2')
    drawLine(xDispLeft, yDispTop, xDispRight, yDispTop, '2')
	drawLine(xDispLeft, yDispTop-1, xDispRight, yDispTop-1, '2')
    drawLine(xDispLeft, yDispMid, xDispRight, yDispMid, '2')
	drawLine(xDispLeft, yDispMid-1, xDispRight, yDispMid-1, '2')
  ...
  elseif object.t == "flagLeft" then
    ...
    drawLine(xDispLeft, yDispMid, xDispLeft, yDispTop, '3')  
	drawLine(xDispLeft-1, yDispMid, xDispLeft-1, yDispTop, '3')  
	
	drawLine(xDispRight, yDispBottom, xDispRight, yDispTop, '3')
	drawLine(xDispRight+1, yDispBottom, xDispRight+1, yDispTop, '3')
    drawLine(xDispLeft, yDispTop, xDispRight, yDispTop, '3')
	drawLine(xDispLeft, yDispTop-1, xDispRight, yDispTop-1, '3')
    drawLine(xDispLeft, yDispMid, xDispRight, yDispMid, '3')
	drawLine(xDispLeft, yDispMid-1, xDispRight, yDispMid-1, '3')
  ...
  elseif object.t == "flagRight" then
    ...
    xDispMarker = ((x - flag.w*2) * zScale) / z + LCD_W/2
    yDispMarker = ((y - gate.h*3/2) * zScale) / z + LCD_H/2
    drawLine(xDispLeft, yDispBottom, xDispLeft, yDispTop, '4')
	drawLine(xDispLeft-1, yDispBottom, xDispLeft-1, yDispTop, '4')
    drawLine(xDispRight, yDispMid, xDispRight, yDispTop, '4')
	drawLine(xDispRight+1, yDispMid, xDispRight+1, yDispTop, '4')
    drawLine(xDispLeft, yDispTop, xDispRight, yDispTop, '4')
	drawLine(xDispLeft, yDispTop-1, xDispRight, yDispTop-1, '4')
    drawLine(xDispLeft, yDispMid, xDispRight, yDispMid, '4')
	drawLine(xDispLeft, yDispMid-1, xDispRight, yDispMid-1, '4')
...
local function run_func(event)
  if not raceStarted then
    lcd.clear()
    lcd.drawText(LCD_W/2 - 64, 80, "Press [Enter] to start")
    if counter then
      lcd.drawText(LCD_W/2 - 35, 28, "Result:")
      lcd.drawNumber(LCD_W/2 + 20, 28, counter, BOLD)
      if isNewBest then
        lcd.drawText(LCD_W/2 - 60, 2, "New best score!")
      else
        lcd.drawText(LCD_W/2 - 60, 2, "Best score:")
        lcd.drawNumber(LCD_W/2 + 30, 2, bestResult)
      end
    else
      lcd.drawText(LCD_W/2 - 125, 28, "Lua FPV Simulator", MIDSIZE+BOLD+ORANGE)
      lcd.drawText(LCD_W/2 - 60, 120, "Best score:")
      if bestResult ~= nil then
		lcd.drawNumber(LCD_W/2 + 30, 120, bestResult)	
	  end
    end
	...
	lcd.drawFilledRectangle( 0,LCD_H/2, 480, LCD_H, BRIGHTGREEN )
	lcd.drawFilledRectangle( 0,0, 480, LCD_H/2, lcd.RGB(0, 136, 200) )
    currentTime = getTime()
    ...
    elseif currentTime < finishTime then
      if (currentTime - startTime) < 100 then
        lcd.drawText(LCD_W/2 - 12, 48, 'GO!', BOLD + MIDSIZE )
